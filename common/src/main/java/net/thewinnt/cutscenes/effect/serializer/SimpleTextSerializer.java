...
public class SimpleTextSerializer implements CutsceneEffectSerializer<SimpleTextConfiguration> {
    ...
    @Override
    public SimpleTextConfiguration fromJSON(JsonObject json) {
        ...
        Easing scale = Easing.fromJSON(json.get("scale"), ConstantEasing.ONE);
        Easing rotation = Easing.fromJSON(json.get("rotation"), ConstantEasing.ZERO);
        Optional<DynamicColor> colorOverride = Optional.ofNullable(DynamicColor.fromJSON(json.get("color_override"), null));
        return new SimpleTextConfiguration(text, rx, ry, centered, scale, rotation, colorOverride);
    }
